...
const FyImage = ({
  ...
  const getDimensionsForWidth = (width) => {

    const ar = aspectRatio || 1; // fallback
    return {
      width,
  ...
  const getUrl = (width, url = src) => {
    if (!isResizable()) {
      return url; // or "" if you strictly want no resize for svg/gif
    }
    ...
    const { width: w, height: h } = getDimensionsForWidth(width);

    // depending on transformImage API – adjust this to your actual implementation
    return transformImage(url, w, h);
    // or transformImage(url, { width: w, height: h, fit: "cover" })
  };
  ...
  return (
    ...
    >
      ...
      <motion.div
        ref={ref}
        initial={isClient && !disableAnimation ? { opacity: 0, y: 15 } : null} // Initial state: transparent and slightly below
        animate={
          isClient && !disableAnimation
            ? { opacity: isInView ? 1 : 0, y: isInView ? 0 : 15 }
            : null
