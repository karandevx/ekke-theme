...
const useAddToCartModal = ({ fpi, pageConfig }) => {
  ...
  function getAvailableActiveSize(sizes) {
    return (
        ...
        const productPriceData = await fpi.executeGQL(
          FEATURE_PRODUCT_SIZE_PRICE,
          {
            ...
            size:
              size ||
              selectedSize ||
              getAvailableActiveSize(productData?.product?.sizes),
          },
      ...
      try {
        ...
        if (
          ...
          const productPriceData = await fetchProductPrice(
            getAvailableActiveSize(productDetails?.data?.product?.sizes),
            productDetails?.data?.product?.slug
          );
          setSelectedSize(
            getAvailableActiveSize(productDetails?.data?.product?.sizes)
          );
          //          // productprice is not available
          setProductData({
  ...
  const handleAddToCart = useCallback(
    async (productSlug) => {
      if (isLoading || isOpen) return; // guard against double invocation
      setIsLoading(true);
      setSlug(productSlug);
    ...
    [fetchProductData, slug, isLoading, isOpen]
  );

  const onSizeSelection = useCallback(
    async (sizeValue, slug) => {
      ...
      const productPriceData = await fetchProductPrice(sizeValue, slug);
        ...
        return fpi.executeGQL(ADD_TO_CART, payload).then((outRes) => {
          if (outRes?.data?.addItemsToCart?.success) {
            ...
            fireCustomGtmEvent("cart.customAdd", {
              cartItem: outRes.data.addItemsToCart.cart,
            });
            if (buyNow) {
              navigate(
  ...
  const selectedSizeDetails = useMemo(() => {
    return productData?.product?.sizes?.sizes?.find(
      ({ value, is_available }) => selectedSize === value && is_available
    );
  }, [selectedSize]);
